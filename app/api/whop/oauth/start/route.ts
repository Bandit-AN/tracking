...
export async function GET(request: Request) {
  ...
  const params = new URLSearchParams({ client_id: config.appId, redirect_uri: whopCallbackUrl(request), response_type: "code", scope: "openid profile email stats:read", state: `${state}.${flowId}`, nonce: browserNonce, code_challenge: await whopPkceChallenge(verifier), code_challenge_method: "S256" });
