import { canAccessWorkspace, forbiddenResponse, getAuthenticatedUser, unauthorizedResponse } from "@/lib/auth";
...
export async function GET(request: Request) {
  ...
  const runtime = await runtimeEnv(); const email = user.email; const admin = user.role === "admin" && isAdmin(runtime, email); const workspaceId = Number(new URL(request.url).searchParams.get("workspaceId") || 1); let rows: Record<string, unknown>[] = [];
  if (!admin && !canAccessWorkspace(user, workspaceId)) return forbiddenResponse();
...
export async function POST(request: Request) {
  const user = await getAuthenticatedUser(request.headers); if (!user) return unauthorizedResponse();
  const runtime = await runtimeEnv(); const body = await request.json() as { workspaceId: number; workspaceName: string; message: string }; const email = user.email; const id = Date.now(); const createdAt = new Date().toISOString();
...
export async function PATCH(request: Request) {
  const user = await getAuthenticatedUser(request.headers); if (!user) return unauthorizedResponse();
  const runtime = await runtimeEnv(); if (user.role !== "admin" || !isAdmin(runtime, user.email)) return forbiddenResponse(); const body = await request.json() as { id: number; status: "open" | "resolved" }; const resolved = body.status === "resolved" ? new Date().toISOString() : null;
