...
type WhopCampaign = { id: string; title?: string; status?: string; spend?: number; spend_currency?: string; impressions?: number; reach?: number; clicks?: number; leads?: number; submitted_applications?: number; purchases?: number; purchase_value?: number; results?: number };
...
function rangeStart(range: string, now: Date) {
  ...
  if (range === "Month to date") start.setDate(1);
  if (range === "Quarter to date") start.setMonth(Math.floor(start.getMonth() / 3) * 3, 1);
...
export function Dashboard({ user }: { user: AuthUser }) {
  ...
  const [modal, setModal] = useState<"client" | "member" | "sheet" | "meta" | "whop" | "support" | "settings" | "payout" | null>(null); const [toast, setToast] = useState("");
  ...
  const [whopCampaigns, setWhopCampaigns] = useState<WhopCampaign[]>([]); const [whopConnected, setWhopConnected] = useState(false); const [whopStatus, setWhopStatus] = useState<"idle" | "loading" | "error">("idle"); const [whopAccount, setWhopAccount] = useState(""); const [whopError, setWhopError] = useState("");
  ...
  const activeSheetUrl = sheetUrls[clientId] ?? "";
  ...
  async function fetchSheetMetrics(url: string, workspaceId: number, force = false): Promise<SheetMetrics> {
      ...
      const applicationSheetDates = applicationData.map((row) => rowValue(row, applicationHeaders, ["submitted_at", "Submitted At", "Timestamp"], 0)).filter((date) => !!parseSheetDate(date));
      const eventApplicationDates = events.filter((row) => row[1]?.trim() === "application_submitted" && parseSheetDate(row[0])).map((row) => row[0]);
      ...
      return {
        ...
        applicationDates: applicationSheetDates.length ? applicationSheetDates : eventApplicationDates,
  ...
  useEffect(() => { const timer = window.setTimeout(() => { void loadSheet(activeSheetUrl); void loadWorkspace(); void loadSupport(); }, 0); return () => window.clearTimeout(timer); }, [clientId, activeSheetUrl]);
  useEffect(() => { const timer = window.setTimeout(() => { void loadMeta(); void loadWhop(); }, 0); return () => window.clearTimeout(timer); }, [clientId, range, customStart, customEnd]);
  const notify = (text: string) => { setToast(text); window.setTimeout(() => setToast(""), 2800); };
  useEffect(() => {
    const params = new URLSearchParams(window.location.search); const metaResult = params.get("meta"); const whopResult = params.get("whop"); if (!metaResult && !whopResult) return;
  ...
  async function refreshDashboard() {
    ...
    const [sheetResult] = await Promise.all([loadSheet(currentSheetUrl, true), loadWorkspace(), loadMeta(), loadWhop()]);
    setRefreshing(false);
  ...
  async function loadWhop() {
    const { start, end } = selectedBounds(); setWhopStatus("loading");
    try {
      const workspaceIds = clientId === agencyClient.id ? clients.filter((item) => item.id !== agencyClient.id).map((item) => item.id) : [clientId];
      const results = await Promise.all(workspaceIds.map(async (workspaceId) => { const response = await fetch(`/api/whop?workspaceId=${workspaceId}&since=${start.toISOString().slice(0, 10)}&until=${end.toISOString().slice(0, 10)}`, { cache: "no-store" }); const data = await response.json() as { connected?: boolean; accountId?: string; accountName?: string; campaigns?: WhopCampaign[]; error?: string }; return { ok: response.ok, ...data }; }));
      const connected = results.filter((result) => result.connected); setWhopConnected(connected.length > 0); setWhopAccount(clientId === agencyClient.id ? `${connected.length} Whop account${connected.length === 1 ? "" : "s"}` : connected[0]?.accountName || connected[0]?.accountId || ""); setWhopCampaigns(connected.flatMap((result) => result.campaigns ?? [])); setWhopError(results.find((result) => result.error)?.error || ""); setWhopStatus(results.every((result) => result.ok) ? "idle" : "error");
    } catch { setWhopStatus("error"); setWhopError("Whop Ads request failed"); setWhopCampaigns([]); }
  }

  function connectWhop(e: React.FormEvent) { e.preventDefault(); if (clientId === agencyClient.id) { setModal(null); notify("Select an offer before connecting its Whop account"); return; } setWhopStatus("loading"); window.location.assign(`/api/whop/oauth/start?workspaceId=${clientId}`); }
  ...
  const whopSpend = useMemo(() => whopCampaigns.reduce((sum, row) => sum + Number(row.spend || 0), 0), [whopCampaigns]);
  const whopDeals = useMemo(() => period.dated.filter(({ deal }) => /whop/i.test(`${deal.source} ${deal.medium} ${deal.campaign}`)), [period]);
  const whopRevenue = whopDeals.reduce((sum, { deal }) => sum + deal.offer, 0); const whopCash = whopDeals.reduce((sum, { deal }) => sum + deal.cash, 0);
  ...
  return <main className="app-shell">
    <aside className={`sidebar ${sidebarOpen ? "open" : ""}`}>
      ...
      <div className="sidebar-bottom"><button className="help" onClick={() => setModal("support")}><span>?</span><div><strong>Need help?</strong><small>Message MoonRift Media</small></div></button><div className="profile"><span>{initials(user.displayName)}</span><div><strong>{user.displayName}</strong><small>{user.role === "admin" || isAgencyAdmin ? "MoonRift agency admin" : `${user.role[0].toUpperCase()}${user.role.slice(1)} account`}</small></div><button className="signout-button" onClick={() => void signOut()} aria-label="Sign out of MoonRift" title="Sign out">↪</button></div></div>
    </aside>
    ...
    <section className="content"><header className="topbar"><button className="menu-btn" onClick={() => setSidebarOpen(true)}>☰</button><div className="crumb"><span>Dashboards</span><b>/</b><strong>{tab}</strong></div><div className="top-actions">{clientId !== agencyClient.id && <button className="invite-btn" onClick={() => setModal("member")}>＋ Invite member</button>}</div></header>
      <div className="dashboard"><div className="page-title"><div><h1>{client.name} <span>{tab}</span></h1></div><div className="title-actions"><button onClick={() => void refreshDashboard()} disabled={refreshing}>{refreshing ? "…" : "↻"} <span>{refreshing ? "Refreshing" : "Refresh data"}</span></button>{clientId !== agencyClient.id && <><button onClick={() => setActionMenu(!actionMenu)}>⋯</button>{actionMenu && <div className="action-menu"><button onClick={openSettings}>Workspace settings</button><button onClick={() => setModal("sheet")}>Manage data source</button></div>}</>}</div></div>
        ...
        {tab === "Overview" && <>{clientId === agencyClient.id && <section className="agency-overview"><div className="agency-overview-head"><div><span>AGENCY PERFORMANCE</span><h2>Every offer in one clear view</h2></div><strong>{agencyOfferRows.filter((row) => row.connected).length} of {agencyOfferRows.length} connected</strong></div><div className="agency-offer-grid">{agencyOfferRows.map((row) => <button key={row.client.id} onClick={() => { setClientId(row.client.id); setActiveNav("Dashboard"); setTab("Overview"); }}><span className="agency-offer-avatar" style={{ background: row.client.color }}>{row.client.avatar ? <img src={row.client.avatar} alt="" /> : row.client.initials}</span><div><h3>{row.client.name}</h3><small>{row.connected ? `${range} performance` : "Connect a Google Sheet"}</small></div><dl><div><dt>Cash</dt><dd>{money(row.cash)}</dd></div><div><dt>Revenue</dt><dd>{money(row.revenue)}</dd></div><div><dt>Closed</dt><dd>{row.closed}</dd></div><div><dt>Show rate</dt><dd>{row.showRate.toFixed(1)}%</dd></div></dl><em>View offer →</em></button>)}</div></section>}<div className="charts"><Chart data={period.cashSeries} labels={period.labels} color="#8b6cff" fill="rgba(139,108,255,.28)" label="Cash collected by payment date" total={money(period.cash)} /><Chart data={period.revenueSeries} labels={period.labels} color="#38d6b6" fill="rgba(56,214,182,.22)" label="Revenue generated by payment date" total={money(period.revenue)} /></div>
          ...
          <div className="metrics-section"><div className="metrics-title"><div><span>PAID AD METRICS</span><h2>Meta performance based on CRM-attributed cash collected</h2></div>{!metaConnected && <button onClick={() => setModal("meta")}>Connect Meta Ads</button>}</div><div className="paid-grid"><article><span>Ad spend</span><strong>{metaConnected ? money(adSpend) : "—"}</strong><small>{metaStatus === "loading" ? "Refreshing Meta data…" : "Pulled from Meta Ads"}</small></article><article><span>Meta-attributed revenue</span><strong>{money(metaRevenue)}</strong><small>From Closed Deals source attribution</small></article><article><span>Meta-attributed cash</span><strong>{money(metaCash)}</strong><small>Cash collected from Meta-sourced deals</small></article><article><span>CRM ROAS</span><strong>{metaConnected && adSpend ? `${(metaCash / adSpend).toFixed(2)}×` : "—"}</strong><small>Attributed cash collected ÷ Meta ad spend</small></article></div></div>
          <div className="metrics-section whop-metrics"><div className="metrics-title"><div><span>WHOP ADS METRICS</span><h2>Whop performance based on CRM-attributed cash collected</h2></div>{!whopConnected && <button onClick={() => setModal("whop")}>Connect Whop Ads</button>}</div><div className="paid-grid"><article><span>Ad spend</span><strong>{whopConnected && whopStatus !== "error" ? money(whopSpend) : "—"}</strong><small>{whopStatus === "loading" ? "Refreshing Whop data…" : whopStatus === "error" ? whopError || "Whop Ads data unavailable" : whopCampaigns.length ? `Pulled from ${whopCampaigns.length} Whop campaign${whopCampaigns.length === 1 ? "" : "s"}` : "No Whop campaign spend in this period"}</small></article><article><span>Whop-attributed revenue</span><strong>{money(whopRevenue)}</strong><small>From Closed Deals source attribution</small></article><article><span>Whop-attributed cash</span><strong>{money(whopCash)}</strong><small>Cash collected from Whop-sourced deals</small></article><article><span>CRM ROAS</span><strong>{whopConnected && whopStatus !== "error" && whopSpend ? `${(whopCash / whopSpend).toFixed(2)}×` : "—"}</strong><small>Attributed cash collected ÷ Whop ad spend</small></article></div></div>
    ...
    {modal && <div className="modal-wrap" role="dialog" aria-modal="true"><button className="modal-backdrop" onClick={() => setModal(null)} aria-label="Close" /><form className="modal" onSubmit={modal === "client" ? addClient : modal === "member" ? invite : modal === "sheet" ? connectSheet : modal === "meta" ? connectMeta : modal === "whop" ? connectWhop : modal === "support" ? sendSupport : modal === "settings" ? saveSettings : addPayout}><button type="button" className="modal-close" onClick={() => setModal(null)}>×</button><span className={`modal-icon ${modal === "whop" ? "whop-modal-icon" : ""}`}>{modal === "payout" ? "$" : modal === "support" ? "?" : modal === "meta" ? "f" : modal === "whop" ? "W" : modal === "settings" ? "⚙" : modal === "sheet" ? "▦" : "＋"}</span>
      <h2>{modal === "client" ? "Create workspace" : modal === "member" ? "Invite team member" : modal === "sheet" ? "Connect Google Sheets" : modal === "meta" ? "Connect Meta Ads" : modal === "whop" ? "Connect Whop Ads" : modal === "support" ? "Message MoonRift Media" : modal === "settings" ? "Workspace settings" : "Add payout"}</h2>
      {modal === "client" && <><label>Workspace name</label><input value={newName} onChange={(e) => setNewName(e.target.value)} required /></>}
      {modal === "member" && <><p>This sends a secure account request to the MoonRift agency inbox. Access is not active until the email, role, and assigned workspace are approved.</p><label>Email address</label><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required /><label>Access level</label><select value={memberRole} onChange={(e) => setMemberRole(e.target.value)}><option>Viewer</option><option>Editor</option><option>Admin</option></select></>}
      ...
      {modal === "meta" && <><p>Sign in with Facebook, approve reporting access, and choose the ad account for this workspace. MoonRift calculates ROAS from your CRM-attributed cash collected—not Meta&apos;s purchase value.</p><div className="meta-oauth-permissions"><b>MoonRift will be able to:</b><span>✓ View ad accounts you can access</span><span>✓ Read campaign spend and performance</span><span>✓ Match Meta spend to CRM-attributed cash collected</span><small>Read-only connection. MoonRift cannot create, edit, publish, or pause ads.</small></div></>}
      {modal === "whop" && <><p>Sign in with Whop, approve read-only reporting access, and choose the business account for this workspace. MoonRift calculates CRM ROAS from Whop spend and CRM-attributed cash collected.</p><div className="meta-oauth-permissions"><b>MoonRift will be able to:</b><span>✓ View Whop business accounts you can access</span><span>✓ Read campaign spend and performance</span><span>✓ Match Whop spend to CRM-attributed cash collected</span><small>Read-only reporting connection. MoonRift cannot create, edit, or pause Whop campaigns.</small></div></>}
      ...
      <div className="modal-actions"><button type="button" onClick={() => setModal(null)}>Cancel</button><button type="submit">{modal === "payout" ? "Record payout" : modal === "support" ? "Send message" : modal === "meta" ? "Continue with Facebook" : modal === "whop" ? "Continue with Whop" : modal === "settings" ? "Save changes" : modal === "sheet" ? "Connect & sync" : modal === "member" ? "Request access" : "Create workspace"}</button></div>
    </form></div>}
